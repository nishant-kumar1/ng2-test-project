	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				...
				selBox +="<select class='ui-pg-selbox' role='listbox'>";
				for(i=0;i<ts.p.rowList.length;i++){
					selBox +="<option role=\"option\" value=\""+ts.p.rowList[i]+"\""+((ts.p.rowNum === ts.p.rowList[i])?" selected=\"selected\"":"")+">"+ts.p.rowList[i]+"</option>";
				}
				selBox +="</select>";
				
				//adding the dropdown
				
				$("#"+$.jgrid.jqID(ts.p.id)).parents('.customerTable').find('.rowCounting').children('div').append(selBox);
				str +=selBox+"</td>";
			}
			...
			//MAKING THE JQGRID DROP DOWN TABLE SPECIFIC
			var UIelem = $("#"+$.jgrid.jqID(ts.p.id)).parents('.customerTable').find('.ui-pg-selbox')
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += imgs;
			thead += ts.p.colNames[i];			
			
			if(!ts.p.colModel[i].width)  { ts.p.colModel[i].width = 150; }
			else { ts.p.colModel[i].width = parseInt(ts.p.colModel[i].width,10); }
			if(typeof ts.p.colModel[i].title !== "boolean") { ts.p.colModel[i].title = true; }
			if(ts.p.colModel[i].hasFilter){thead += '<i data-rel="gridFilter" data-filname="'+ts.p.colModel[i].name+'" class="icons filtericon">&nbsp;</i>';}
		...
		$("th",thr).each(function ( j ) {
			...
			//THE FILTER FUNCTION PLUGGED IN HERE TO OVERIDE THE DEFAULT DISABLITY OF FUNCTION
			
			
			var target = (e.target!=undefined)?$(e.target):$(e.srcElement);
			
			if(target.attr('data-rel')!=undefined && 
			   target.attr('data-rel')=='gridFilter'){
				ts.p.onFilterClick(e);
				return false;
			}
